...
async function testSendToAll() {
  ...
  /* ---- تحقق إضافي (send-to-all): توازي، أنواع غير متوقعة، فشل مخزن العدّاد، عدم تسريب تفاصيل ---- */
  await check('طلبات متوازية (20 خاطئة دفعة وحدة): 5 كحد أقصى كتوصل للمقارنة والباقي 429', async () => {
    const { h, st } = mk();
    const rs = await Promise.all(Array.from({ length: 20 }, (_, i) => call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: 'burst-wrong-' + i } }))));
    const c403 = rs.filter(r => r.statusCode === 403).length, c429 = rs.filter(r => r.statusCode === 429).length;
    assert.ok(c403 <= 5 && c403 + c429 === 20, `403=${c403} 429=${c429}`); assert.equal(st.sent.length, 0); });
  await check('pin من نوع غير نصي (object/array/number/null/true) => 403 بلا كراش وبلا إرسال', async () => {
    for (const bad of [{ a: 1 }, ['x'], 12345678, null, true, 'x'.repeat(300)]) {
      const { h, st } = mk(); const r = await call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: bad } }));
      assert.equal(r.statusCode, 403, 'pin=' + JSON.stringify(bad).slice(0, 20)); assert.equal(st.sent.length, 0); } });
  await check('verifyOnly لازم يكون boolean true بالضبط ("true" نصي => يعتبر إرسال عادي => 400 بلا عنوان/نص)', async () => {
    const { h, st } = mk(); const r = await call(h, req({ headers: bearer('tA'), body: { pin: GOOD_PIN, verifyOnly: 'true' } }));
    assert.equal(r.statusCode, 400); assert.equal(st.sent.length, 0); });
  await check('الرمز صحيح لكن title/body ناقصين أو غير نصيين أو طويلين => 400 وبلا إرسال', async () => {
    for (const b of [{}, { title: 't' }, { body: 'b' }, { title: 5, body: 'b' }, { title: 't', body: ['b'] }, { title: '  ', body: 'b' }, { title: 'x'.repeat(201), body: 'b' }, { title: 't', body: 'x'.repeat(1001) }]) {
      const { h, st } = mk(); const r = await call(h, req({ headers: bearer('tA'), body: { ...b, pin: GOOD_PIN } }));
      assert.equal(r.statusCode, 400, JSON.stringify(b).slice(0, 40)); assert.equal(st.sent.length, 0); } });
  await check('claim storeId شكلو غير صالح (../store_b, حروف كبيرة, non-string) => 403 وبلا إرسال', async () => {
    for (const bad of ['../store_b', 'Store_A', 'a/b', 7, '']) {
      const st = makeState({ tokens: { tX: { uid: 'uX', storeId: bad } }, db: { 'stores/store_b/fcmTokens': { devB1: {} } } });
      const h = loadHandler('api/send-to-all.js', st, { BROADCAST_PIN: GOOD_PIN });
      const r = await call(h, req({ headers: bearer('tX'), body: { title: 't', body: 'b', pin: GOOD_PIN } }));
      assert.equal(r.statusCode, 403); assert.equal(st.sent.length, 0); } });
  await check('مخزن عدّاد المحاولات تعطّل => fail-closed: 500 وبلا إرسال (حتى بالرمز الصحيح)', async () => {
    const { h, st } = mk(); const realDb = st.admin.database;
    st.admin.database = () => { const d = realDb(); return { ref: (p) => (p.startsWith('_security/') ? { transaction: async () => { throw new Error('db down'); }, once: async () => { throw new Error('db down'); }, remove: async () => {} } : d.ref(p)) }; };
    const r = await call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: GOOD_PIN } }));
    assert.equal(r.statusCode, 500); assert.equal(st.sent.length, 0); });
  await check('خطأ داخلي فالإرسال => رد عام بلا تفاصيل، والرد ما فيه الرمز أبدًا', async () => {
    const { h, st } = mk(); st.admin.messaging = () => ({ sendEachForMulticast: async () => { throw new Error('secret-internal-detail'); } });
    const r = await call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: GOOD_PIN } }));
    assert.equal(r.statusCode, 500); const j = JSON.stringify(r.body); assert.ok(!j.includes('secret-internal-detail')); assert.ok(!j.includes(GOOD_PIN)); });
  await check('Cache-Control: no-store + رمز خاطئ لا يظهر فالرد', async () => {
    const { h } = mk(); const r = await call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: 'wrong-pin-xyz-123' } }));
    assert.equal(r.headers['Cache-Control'], 'no-store'); assert.ok(!JSON.stringify(r.body).includes('wrong-pin-xyz-123')); });
}
