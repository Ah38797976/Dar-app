...
function makeState(overrides = {}) {
  ...
  // asyncDb=true: كل عملية قاعدة بيانات تتنازل للـ event loop، فالطلبات المتوازية كتتداخل فعليًا (كيف كيوقع مع Vercel)
  st.tick = () => (st.asyncDb ? new Promise((r) => setImmediate(r)) : Promise.resolve());
  st.admin = {
    apps: [],
    ...
    database: () => ({
      ref: (p) => ({
        once: async () => { await st.tick(); return { val: () => (st.db[p] === undefined ? null : st.db[p]) }; },
        // يحاكي Firebase: الدالة تُنفَّذ ذرّيًا على القيمة الحالية وقت الكتابة؛ إذا رجعت undefined => abort بلا أي تغيير
        transaction: async (fn) => {
          await st.tick();
          const cur = st.db[p] === undefined ? null : JSON.parse(JSON.stringify(st.db[p]));
          const next = fn(cur);
          if (next === undefined) return { committed: false, snapshot: { val: () => cur } };
          st.db[p] = next;
          return { committed: true, snapshot: { val: () => next } };
        },
        remove: async () => { await st.tick(); delete st.db[p]; }
      })
    }),
    messaging: () => ({
...
async function testSendToAll() {
  ...
  await check('انتهاء القفل بعد 15 دقيقة: الرمز الصحيح يمرّ والعداد يُصفَّر (وتنحل الحالة العالقة القديمة)', async () => {
    const { h, st } = mk();
    const realNow = Date.now; let offset = 0; Date.now = () => realNow() + offset;
    try {
      const go = (pin) => call(h, req({ headers: bearer('tA'), body: { pin, verifyOnly: true } }));
      for (let i = 0; i < 5; i++) assert.equal((await go('bad-pin-' + i + 'xx')).statusCode, 403);
      assert.equal((await go(GOOD_PIN)).statusCode, 429);                      // مقفول (حتى بالرمز الصحيح)
      offset = 14 * 60 * 1000;                                                 // بعد 14 دقيقة: ما زال مقفولًا
      let r = await go(GOOD_PIN); assert.equal(r.statusCode, 429);
      assert.ok(Number(r.headers['Retry-After']) > 0 && Number(r.headers['Retry-After']) <= 60);
      offset = 15 * 60 * 1000 + 1000;                                          // بعد 15 دقيقة: القفل انتهى
      assert.equal((await go(GOOD_PIN)).statusCode, 200);
      assert.equal(st.db['_security/broadcast/uA'], undefined);                // العداد والقفل تصفّرا
      for (let i = 0; i < 5; i++) assert.equal((await go('bad-again-' + i + 'x')).statusCode, 403); // عدّاد جديد من الصفر
      assert.equal((await go('bad-again-6xx')).statusCode, 429);
      // حالة عالقة ورثناها من النسخة القديمة: attempts=5 مع lockedUntil فالماضي => تنحل بالرمز الصحيح
      st.db['_security/broadcast/uA'] = { attempts: 5, lockedUntil: Date.now() - 1000 };
      assert.equal((await go(GOOD_PIN)).statusCode, 200);
    } finally { Date.now = realNow; } });
  await check('طلبات متوازية: 30 تخمينًا خاطئًا معًا => 5 فقط تُفحص (403) والباقي 429، ولا يدخل الرمز الصحيح بعدها', async () => {
    const { h, st } = mk({ asyncDb: true });
    const rs = await Promise.all(Array.from({ length: 30 }, (_, i) =>
      call(h, req({ headers: bearer('tA'), body: { pin: 'bad-parallel-' + i, verifyOnly: true } }))));
    const n403 = rs.filter((r) => r.statusCode === 403).length, locked = rs.filter((r) => r.statusCode === 429);
    assert.equal(n403, 5); assert.equal(locked.length, 25);
    assert.ok(locked.every((r) => Number(r.headers['Retry-After']) > 0));
    assert.equal(st.db['_security/broadcast/uA'].attempts, 5);
    const r = await call(h, req({ headers: bearer('tA'), body: { title: 't', body: 'b', pin: GOOD_PIN } }));
    assert.equal(r.statusCode, 429); assert.equal(st.sent.length, 0); });
